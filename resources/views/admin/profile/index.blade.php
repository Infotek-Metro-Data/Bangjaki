<x-layouts.admin title="Profil Saya">
    ...
    <div class="max-w-4xl mx-auto p-4 md:p-6 lg:p-8 flex flex-col gap-6">
        ...
        <div class="flex items-center gap-2 text-sm text-gray-500">
            <a href="{{ route('admin.dashboard') }}" class="hover:text-orange-600 hover:underline">Home</a>
            <span>/</span>
        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
            <div>
                <h1 class="text-3xl font-black tracking-tight text-slate-900 mb-2">Profil Admin</h1>
                <p class="text-gray-500">Kelola detail akun dan pengaturan keamanan Anda.</p>
            </div>
            <div
                class="flex items-center gap-2 px-3 py-1.5 bg-white border border-gray-100 rounded-lg shadow-sm text-sm text-gray-600">
        ...
        <div class="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
            ...
            <form id="profileForm" action="{{ route('admin.profile.update') }}" method="POST"
                enctype="multipart/form-data" class="p-6 md:p-8 border-b border-gray-50">
                @csrf
                <div class="flex items-center justify-between mb-6">
                    <h2 class="text-xl font-bold text-slate-900">Informasi Pribadi</h2>
                </div>
                    ...
                    <div class="flex flex-col items-center gap-3 min-w-[120px]">
                        <div class="relative group cursor-pointer w-24 h-24">
                            <div class="w-full h-full rounded-full overflow-hidden border-4 border-gray-50 shadow-md">
                                @if ($user->foto_profil)
                                    <img id="avatarPreview" src="{{ asset('storage/' . $user->foto_profil) }}"
                                        alt="Avatar" class="w-full h-full object-cover">
                                @else
                                    <div id="avatarPlaceholder"
                                        class="w-full h-full bg-orange-100 flex items-center justify-center text-orange-600 font-bold text-3xl">
                            ...
                            <div class="absolute inset-0 bg-slate-900/40 rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity"
                                onclick="document.getElementById('fotoInput').click()">
                                <span class="material-symbols-outlined text-white">edit</span>
                            </div>

                            <button type="button" onclick="document.getElementById('fotoInput').click()"
                                class="absolute bottom-0 right-0 bg-orange-500 text-white rounded-full p-1.5 border-2 border-white shadow-sm hover:bg-orange-600 transition-colors">
                    ...
                    <div class="flex-1 w-full grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="flex flex-col gap-2">
                            ...
                            <input type="text" name="nama" value="{{ old('nama', $user->nama) }}"
                                class="w-full px-4 py-2.5 rounded-lg border border-gray-200 focus:ring-2 focus:ring-orange-500 focus:border-orange-500 outline-none text-slate-900 placeholder:text-gray-400 transition-all bg-white">
                        </div>
                        <div class="flex flex-col gap-2">
                        ...
                        <div class="md:col-span-2">
                            ...
                            <input type="text"
                                value="{{ $user->peran == 'admin' ? 'Administrator (Full Access)' : 'Petugas' }}"
                                disabled
                                class="w-full px-4 py-2.5 rounded-lg border border-gray-100 bg-gray-50 text-gray-500 cursor-not-allowed">
                        </div>

                        <div class="md:col-span-2 flex justify-end">
                            <button type="submit"
                                class="bg-orange-500 hover:bg-orange-600 text-white px-6 py-2.5 rounded-lg font-bold shadow-lg shadow-orange-500/20 transition-all active:scale-95 flex items-center gap-2">
            ...
            <form id="passwordForm" action="{{ route('admin.profile.password') }}" method="POST"
                class="p-6 md:p-8 bg-gray-50/50">
                @csrf
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-3xl">
                    ...
                    <div class="md:col-span-2 flex justify-end mt-2">
                        <button type="submit"
                            class="bg-white border border-gray-200 hover:bg-gray-50 text-slate-700 px-6 py-2.5 rounded-lg font-bold transition-all active:scale-95 flex items-center gap-2">
    ...
    <script>
        ...
        document.getElementById('passwordForm').addEventListener('submit', function(e) {
            ...
            btn.disabled = true;
            btn.innerHTML =
                '<span class="material-symbols-outlined animate-spin text-[20px]">refresh</span> Memproses...';
